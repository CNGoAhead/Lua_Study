...
local heapq = require('Debug.heapq')
...
function BattleMap:clear()
    ...
    self:removeAllObj(self.favBuilds[5], "_bfid5")
...
function BattleMap:hideBattler(battler)
    if battler.avater then
        ...
        if fav>=1 and fav<=2 or fav == 5 then
            self.types[fav] = self.types[fav]-1
            if self.favBuilds[fav] then
                self:removeObj(self.favBuilds[fav], battler, "_bfid" .. fav)
            end
        end
        if fav<4 then
            self.types[0] = self.types[0] - 1
            self:removeObj(self.battler, battler, "_btid")
            self:removeObj(self.build, battler, "_bid")
        end
    end
    self:removeObj(self.battlerAll, battler, "_btaid")
end

function BattleMap:showBattler(battler)
    if battler.avater then
...
--扩展该函数，增加是否强制跳过城墙检测的变量
function BattleMap:canMoveWithDirectMove(sgx, sgy, tgx, tgy, forceSkipWall)
...
--扩展该函数，增加是否强制跳过城墙检测的变量
function BattleMap:canAttackWithDirectMove(sx, sy, soldier, target, forceSkipWall)
    local viewInfo = target.battleViewInfo
    if not viewInfo then
        viewInfo = self:getSoldierBattleViewInfo(target)
    end
