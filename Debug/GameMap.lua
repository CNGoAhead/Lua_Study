...
function GameMap.initGridSetting(baseX, baseY, gsizeX, gsizeY, msizeX, msizeY)
    _baseX = baseX
    _baseY = baseY
    _sizeX = gsizeX
    _sizeY = gsizeY
...
local function _setGrid(x, y, obj)
    ...
    --为了把城墙权重加到battleMap中
    local scene = GMethod.loadScript("scenes.MainScene")
    if obj then
...
-- 两套角度体系
-- 以网格算的方式是左上为x正方向，右上为y正方向
-- 以建筑算的方式是向下为x正方向，左边为y正方向
-- 都不太正常= =
function GameMap.convertToGridAngle(buildAngle)
    return buildAngle - 135
end

function GameMap.convertToBuildAngle(gridAngle)
    return gridAngle + 135
end

function GameMap.computeBuildAngle(ox, oy)
    return (math.deg(math.atan2(oy, ox)) + 495) % 360
end
